...
import TempPumpkins from '../models/tempPumpkinsModel.js';
...
class EventService {
 ...
 /* async getTicketsEvents(userId,event) {
    ...
    try {
      ...
      switch (estado) {
        ...
        case 1:
          ...
          const matchFound = await TempPumpkins.findOne({
            where: {
              user:user,
              estado:1,
              game: type,
          ...
          switch (type) {
            case 3:
              ...
              const numGames = await TempPumpkins.findAll({
        ...
        case 2:
          ...
          const match = await TempPumpkins.findOne({
  ...
  async redeemTicketAndReward(tknGame,opcion,token,modalidad,type,isDataIntegrityValid,paramsString,userId,user2,key1,key2, req) {
    ...
    try {
      ...
      if(!sessionToken){
        await t.rollback(); // Revertir la transacción en caso de error
        return { success: false, code: '005', message: '¡Esta sesión es antigua! No puedes tener más de una sesión abierta para jugar' };
      }
      ...
      if (!allPrizes) {
        await t.rollback(); // Revertir la transacción en caso de error
        return { success: false, code: '302', message: 'No se encontró premios para este juego' };
      }
      ...
      switch (type) {
        ...
        case 5:
          ...
          const userGame = await TempPumpkins.findOne({
        ...
        case 6:
          ...
          const userGame2 = await TempPumpkins.findOne({
            attributes: ['premios','picked'],
                where: {
                    user: userId,
                    game: 5,
    ...
    try {
      ...
      switch (type) {
        ...
        case 4:
          ...
          //Obtener partida en curso
          const match4 = await TempPumpkins.findOne({
            attributes: ['calabazas','premios','picked','nombres'],
            where: {
              user:user,
              estado:1,
