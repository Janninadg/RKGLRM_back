...
class StoreController {
    ...
    async getVirtualInventory(req, res, next) {
      try {
        const { user,token } = req.body;
  
        const response = await StoreService.getVirtualInventory(user,token);
        //console.log(ranking);
        ...
        console.error('Error al obtene virtual items:', error);
        return res.status(500).json({ error: 'Error interno del servidor' });
      }
  }
